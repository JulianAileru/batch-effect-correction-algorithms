...
class MetNorm:    
    ...
    def __init__(self,data,metadata,cv=False,model=None):
        """
        Initialize MetNorm Class
        """

        self.data = data.copy() 
        self.metadata = metadata.copy()
        #Divide dataset based on sample_type 
        ...
        self.normed = None
        self.cv = cv
        # Model can be specified, hyperparameters 
        # will be turned using GridSearchCV when cv=True
        self.model = model if model is not None else SVR(kernel='rbf',gamma='scale', tol=0.001, C=1.0, epsilon=0.1, shrinking=True)
        self.param_grid = {'kernel': ['rbf', 'linear','poly'],
                           'C': [0.1, 1, 10, 100],
